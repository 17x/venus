...
```typescript
type VenusPaint =
  | { type: 'solid'; color: string; opacity?: number; visible?: boolean; blendMode?: VenusBlendMode }
  | { type: 'gradient'; gradient: VenusGradient; opacity?: number; visible?: boolean; blendMode?: VenusBlendMode }
  | { type: 'image'; assetId: string; scaleMode?: 'fill' | 'fit' | 'crop' | 'tile'; opacity?: number; visible?: boolean }
```
...
```typescript
type VenusEffect =
  | { type: 'dropShadow'; color?: string; offsetX?: number; offsetY?: number; blur?: number; visible?: boolean }
...
| FM-P0-004 | Update engine-docs property docs to grouped sections | `apps/engine-docs/src/engineApiDocs.ts`, `App.tsx` | Properties show Identity / Transform / Appearance / Effects / Type-specific |
| FM-P0-005 | Add docs contract for grouped property taxonomy | `engine-docs.contract.test.ts` | Docs cannot regress to one flat string list only |

### P1 — Hit-test and Bounds Alignment

| ID | Task | Files | Acceptance |
| --- | --- | --- | --- |
| FM-P1-001 | Stroke align hit-test semantics | `scene/hitTest`, tests | inside/outside/center have explicit behavior or documented limitation |
| FM-P1-002 | Dash/cap/join hit-test coverage | `scene/hitTest`, tests | line/path stroke tests include dash/cap/join cases |
| FM-P1-003 | Effect bounds policy | `worldBounds`, docs | shadow/blur bounds expansion either implemented or documented as render-only |
| FM-P1-004 | Clip/mask hit-test decision | `scene/hitTest`, docs | mask source/alpha/clip-like rule decided and tested |

### P2 — Renderer Fidelity and Cache

| ID | Task | Files | Acceptance |
| --- | --- | --- | --- |
| FM-P2-001 | Cache key includes structured appearance/effects | `core/cache`, renderer plan tests | paint/effect changes invalidate raster/geometry as expected |
| FM-P2-002 | Canvas2D fidelity texture path docs and diagnostics | renderer, debug docs | inspect shows raster texture/cache cost in user-readable terms |
| FM-P2-003 | WebGL packet LOD hints for structured fields | `renderer/webgl/core/packets.ts` | structured appearance maps to existing feature hints |
| FM-P2-004 | Canvas2D render tests for structured appearance | `renderer/canvas2d`, tests | structured fields render equivalent to legacy fields |
