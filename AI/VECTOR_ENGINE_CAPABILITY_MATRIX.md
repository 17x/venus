...
| `text` + `textRuns` | Exists as `text` + `runs` | Engine | Add adapter parity tests for style fields. |
| `image` + `assetId/assetUrl` | Engine image uses `assetId`; resource loader handles URLs | Engine | Add explicit asset URL map/import path. |
...
| `clipPathId/clipRule` | Engine has `clip.clipNodeId` and `rule` | Engine | Add clip graph validation and parity across render/hit/export. |
...
| --- | --- | --- | --- |
| Hit test | `engine.hitTest(...)`, `engine.hitTestAll(...)` | Same root-level API | Keep. Extend result details and clip policy. |
...
| Clip validation | `validateClipGraph`, `resolveClipDependencies` | Root-level clip graph validation API | Missing/self/cycle/rule/inline checks complete; render/hit/export parity pending. |
...
| --- | --- | --- | --- |
| Base | Vector syncs snapshots and dirty bounds into engine | Engine has base/layered render paths and cache pieces; default base module now owns layer-order API | Continue moving base/active/overlay controls under base while keeping flat root APIs. |
| Overlay | Vector owns overlay model/adapters | Engine has overlay renderer primitives | Extract generic selection/hover/guide overlay model to engine. |
...
7. Add clip graph validation and use it from render, hit-test, and export tests. Root validation API done; parity tests pending.
