...
- `[-]` B3 WebGPU rect-batch 执行切口：rect 全量可用时已切换为 native-first（submit 成功直接返回 `native-rect-batch`），仅在不满足资格或 native submit 失败时回退 WebGL
...
- `[-]` B3 staged geometry 分层：rect 顶点/索引 payload 与 clear 解析已抽离到 `webgpuRectBatchGeometry.ts`，native runtime 仅保留 pass 提交与设备交互职责
- `[-]` B3 staged upload 执行：rect 路径已在 queue 支持时发出 `writeBuffer` 顶点/索引上传，harness 已覆盖上传次数与字节数断言
...
- 透视/正交切换与动画过渡
- 相机状态快照与回放
- 状态：`[-]`（已新增纯 3D camera 控制器基础：orbit/fly/FPS、透视/正交投影切换描述、相机快照与回放 keyframe 选择；runtime 已支持 initial/set/get/clear 3D camera snapshot，并在 diagnostics.camera3d 暴露 controller/projection/pose 摘要；snapshot 已进入 renderer frame context；新增 WebGPU camera uniform plan，将 view/projection/viewProjection/position/target 打包为 224-byte uniform payload，WebGPU stats 可观测 camera3D active/controller/projection/uniform bytes）
...
- 层级遮挡裁剪
- 动态 LOD 分层阈值
- 状态：`[-]`（frustum + occlusion 诊断计数已落地；新增压力感知动态 LOD 阈值规划器并透传到 runtime diagnostics；新增纯层级遮挡规划器，可基于 screen bounds/depth/parent 传播输出 visible/occluded 决策；runtime diagnostics 已新增 visibilityOcclusion rank-proxy 桥接，用现有 frame plan 驱动层级遮挡 planner 画像；真实 camera-projected bounds/depth 接线仍待补齐）
...
- 实例化渲染与批处理
- 状态：`[-]`（WebGPU 已具备每帧原生 submit 轻量通路与诊断计数；新增 WebGPU 3D pass planner，可按 material/lighting/geometry key 生成批次、识别 instanced batch 并输出 nativeCoverage/lit/unlit 覆盖率；WebGPU renderer 每帧 stats 与 runtime diagnostics.webgpu.pass3d 已透出 3D pass candidate/batch/coverage 画像；新增 camera uniform payload 与 WebGPU 3D binding plan，可估算 material/light/instance/camera uniform bytes 并提前稳定 shader upload/bind-group 数据契约；主渲染 pass 的 GPU command 执行仍待替换）
...
- 深度优先命中与选择优先级策略
- 高频交互下命中预算回退机制
- 状态：`[-]`（已完成 ray 命中预算与 miss 分类诊断；native ray hit 已支持 mesh/instance target metadata、rayDistance 深度优先排序与 hitPlan 主目标类型透传；新增纯 mesh/instance ray-triangle 相交器，可输出实例 metadata、rayDistance 与预算耗尽 summary；新增 scene/asset mesh ray candidate adapter，可将 inline geometry 或 mesh asset registry 转成 ray candidates 并显式报告缺失 geometry；与真实场景 mesh 存储/加速结构的 runtime 接线仍待补齐）
...
- 资源优先级与取消策略
- 缓存与显存预算联动
- 状态：`[-]`（已新增 mesh/texture streaming 纯规划器：按可见性/优先级/距离选择加载、取消失效 in-flight、按 LRU 与锁定/当前需求约束做缓存/显存预算驱逐；新增 streaming plan executor，可按 cancel/evict/load 顺序执行 host loader/cache 回调且显式传播错误；execution summary 已包含 load/cancel/evict 计数与 loaded/evicted/retained bytes，并新增 execution diagnostics mapper 供 runtime/dashboard 复用；runtime diagnostics 已暴露资源缓存、上传预算与 streaming backlog 画像；接入真实 renderer mesh/texture residency 仍待补齐）
...
- 各 pass 成本与预算命中率
- 场景级性能画像
- 状态：`[-]`（已补充命中、可见性 LOD、WebGPU、资源/streaming 关键计数观测，并新增 runtime performanceProfile：帧耗时、场景可见/裁剪比例、cache 命中率、pass 成本与预算利用率；WebGPU render stats 与 diagnostics.webgpu.pass3d 已透出 3D pass coverage 画像；新增 diagnostics.webgpu.camera3d（active/controller/projection/uniform bytes/float count）与 diagnostics.webgpu.binding3d（material/light/instance/total uniform bytes）以承接 camera/binding 准备层；WebGPU timestamp-query 能力探测、query lifecycle plan（ready/missing/unsupported 与 queryWriteCount）、native begin/end writeTimestamp、resolveQuerySet/copyBufferToBuffer 尝试计数与 mapped readback parser 已落地，真实 GPU buffer map/readback 调度仍待补齐）
