...
import {Button, Con} from '../../ui/index.ts'
// Keep layer panel contracts sourced from type definitions to avoid runtime hook coupling.
import type {EditorExecutor, LayerItem} from '../../product/useEditorRuntime/types.ts'
    ...
    <section className={'flex h-full w-full min-h-0 flex-col overflow-hidden text-[12px] leading-[18px] text-slate-950 dark:text-slate-100'} role={'region'}>
      ...
      <div className={'scrollbar-custom min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-1 pb-1'}>
        ...
        <div ref={scrollRef}
             ...
             className={'relative min-h-0 flex-1 select-none overflow-x-hidden overflow-y-auto rounded bg-white p-1 dark:bg-slate-900'}>
...
function LayerActionButton(props: {
  ...
  return (
    <Button
      ...
      disabled={disabled}
      onClick={onClick}
      className={'inline-flex h-7 w-full items-center justify-center rounded bg-slate-50 text-slate-700 hover:bg-slate-100 dark:bg-slate-950 dark:text-slate-200 dark:hover:bg-slate-800 disabled:opacity-40'}
    >
      {icon}
    </Button>
  )
