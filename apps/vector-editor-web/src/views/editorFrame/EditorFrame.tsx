...
} from '../../runtime/shell/state/shellState.ts'
import {
  LEGACY_TOOLBELT_MODE_STORAGE_KEY,
  readStoredToolbeltMode,
  TOOLBELT_MODE_STORAGE_KEY,
  type ToolbeltMode,
} from '../../runtime/shell/state/toolbeltState.ts'
      ...
      <FileReceiver executeAction={props.executeAction} resolveDropPosition={(clientX, clientY) => {
        ...
        >
          ...
          >
            ...
            {props.canvas.guideHintLabels && props.canvas.guideHintLabels.length > 0 && (
              // Show the highest-priority engine hint text so users can see line/anchor affordance context.
              <div className={'pointer-events-none absolute left-3 top-16 z-20 rounded-xl border border-sky-200/80 bg-sky-50/90 px-3 py-1 text-xs font-medium text-sky-700 shadow-sm backdrop-blur dark:border-sky-700/70 dark:bg-sky-950/85 dark:text-sky-200'}>
                {props.canvas.guideHintLabels[0]}
              </div>
            )}
            ...
            <MemoEngineViewport
              ...
              transformPreviewActive={props.canvas.transformPreviewActive}
              overlayNodes={props.canvas.overlayNodes}
...
function EditorFrameRuntime() {
  ...
  const createWorker = useCallback(
    () => new Worker(new URL('../../app/editor.worker.ts', import.meta.url), {type: 'module'}),
    [],
  )
  const runtime = useEditorRuntime({createWorker})
