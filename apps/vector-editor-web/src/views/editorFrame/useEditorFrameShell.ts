...
import {createShellCommandDispatch} from '../../runtime/shell/commands/shellCommandDispatch.ts'
import type {InspectorContext, InspectorPanelId} from '../../runtime/shell/state/inspectorState.ts'
import type {ToolbeltMode} from '../../runtime/shell/state/toolbeltState.ts'
...
interface UseEditorFrameShellOptions {
  ...
  setCurrentTool: (tool: import('../../runtime/model/index.ts').ToolName) => void
...
export function useEditorFrameShell(options: UseEditorFrameShellOptions) {
  const dispatchShellCommand = useMemo(() => createShellCommandDispatch({
    onSetZoom(payload) {
      // Clamp shell command input to the runtime zoom envelope so command-driven zoom matches viewport policy.
      options.executeAction('world-zoom', {
        zoomTo: true,
        zoomFactor: Math.max(
          DEFAULT_VIEWPORT_SCALE_RANGE.min,
          Math.min(DEFAULT_VIEWPORT_SCALE_RANGE.max, payload.zoomPercent / 100),
        ),
      })
    },
  ...
  const onSelectTool = useCallback((tool: import('../../runtime/model/index.ts').ToolName, meta: import('../../runtime/shell/commands/shellCommandRegistry.ts').ShellCommandMeta) => {
