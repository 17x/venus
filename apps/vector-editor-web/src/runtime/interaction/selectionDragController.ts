...
export interface SelectionDragController {
  pointerDown: (
    ...
    options?: {
      // Stores optional hinted shape id resolved by hover/click pipeline.
      hitShapeId?: string | null
      // Stores whether group ancestor promotion should apply to pointer-down hits.
      preferGroupSelection?: boolean
      // Stores optional world-space tolerance override so hover/click stay aligned.
      hitTolerance?: number
    },
  ) => boolean
...
/**
 * Creates drag controller that reuses engine hit-test policy for pointer-down and move sessions.
 */
export function createSelectionDragController(options?: {
  ...
  return {
    pointerDown(pointer, snapshot, modifiers, pointerDownOptions) {
      ...
      const preferGroupSelection = pointerDownOptions?.preferGroupSelection ?? false
      ...
      if (hintedId) {
        ...
        const hintedHitId = hintedShape
          ...
          : null
        if (hintedHitId) {
          hitShape = shapeSnapshotById.get(hintedHitId) ?? null
        }
      }
      if (!hitShape) {
    ...
    pointerMove(pointer, snapshot) {
      if (pending) {
        const moved = resolveDragDistance(pending.start, pointer)
        // Keep drag-start threshold policy aligned with shared editor primitive helpers.
        if (!hasPassedDragThreshold(moved, dragThresholdPx)) {
          return {
            phase: 'pending',
            session: null,
          }
