...
import {collectTransformLeafTargets} from '../../runtime/interaction/transformTargets.ts'
 ...
 * @param interactionDocument Canonical document used to expand group transform targets.
 * @param selectedBounds Current selected bounds.
...
export function startTransformSessionFromSelection(input: {
  ...
  const transformTargets = collectTransformLeafTargets(selectedShapes, input.interactionDocument)
  ...
  input.transformManagerRef.current?.start({
    ...
    shapes: transformTargets.map((shape) => createTransformSessionShape(shape)),
    handle: input.handle,
    pointer: input.point,
