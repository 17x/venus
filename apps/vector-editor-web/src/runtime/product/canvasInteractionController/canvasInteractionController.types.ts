...
import type {HoverHitBudgetState} from '../../useEditorRuntime/selectionGroupHelpers.ts'
...
export type PointerScreenPoint = {x: number; y: number}
export type PointerScreenMetadata = {screen: PointerScreenPoint; pointerId: number}
export type PointerPanOrigin = {x: number; y: number; pointerId: number}
export type PointerPanOffset = {x: number; y: number}
...
export interface EditorRuntimeCanvasInteractionControllerState {
  ...
  /** Stores engine-compatible pan origin while the panning tool owns left-button drag. */
  panDragOrigin: PointerPanOrigin | null
  /** Stores pending engine-compatible pan offset while the panning tool owns left-button drag. */
  panDragOffset: PointerPanOffset
}
...
export interface EditorRuntimeCanvasInteractionHandlers {
  ...
  onPointerMove(point: {x: number; y: number}, metadata?: PointerScreenMetadata): void
