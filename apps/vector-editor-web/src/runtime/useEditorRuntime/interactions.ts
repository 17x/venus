...
import {createDocumentNodeFromElement} from '../adapters/fileDocument/fileDocument.ts'
...
export function useEditorRuntimePointerHandlers(input: {
  ...
  const onPointerMove = useCallback((point: {x: number; y: number}, metadata?: {screen: {x: number; y: number}; pointerId: number}) => {
    ...
    input.canvasInteractions.onPointerMove(point, metadata)
  }, [input])
  ...
  const onPointerDown = useCallback((
    ...
    input.lastCanvasPointRef.current = point
    input.worldPointRef.current?.set(point)
    input.canvasInteractions.onPointerDown(point, modifiers, metadata)
  }, [input])
