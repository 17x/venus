...
const TEST_LOD_CONFIG = {
  ...
  interactionCapabilities: {
    zoom: {
      // Keep zoom behavior aligned with the no-degrade policy while running
...
test('zoom interaction stays full quality and keeps overlay fidelity', () => {
  // Keep zoom interactions free of policy-level degradation so startup and
  // active gestures stay readable at near-100% scales.
  const policy = resolveRuntimeRenderPolicy({
    phase: 'zoom',
    lodLevel: 3,
    viewportScale: 0.04,
