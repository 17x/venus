...
import {bindViewportGestures} from '../../interaction/index.ts'
import {publishRuntimeViewportSnapshot} from '../../events/index/index.ts'
import {type RuntimeRenderPhase, type EngineViewportProps} from './engineTypes.ts'
import {DEFAULT_VIEWPORT_SCALE_RANGE} from '../../viewport/index.ts'

// Bridge runtime viewport state into native gesture bindings and renderer-facing interaction signals.
export function EngineViewport({
  ...
  const runtimeRenderPhase = React.useMemo<RuntimeRenderPhase>(() => {
    if (transformPreviewActive) {
      // Continuous preview sessions must keep renderer in precision phase so
      // active layer receives immediate updates until commit-on-release.
      return 'precision'
    }
    ...
    return 'settled'
  }, [editingMode, transformPreviewActive, viewportInteractionType, viewportVelocity])
  ...
  React.useEffect(() => {
    ...
    return bindViewportGestures({
      ...
      // Keep native wheel/pinch zoom range aligned with runtime/controller clamp policy.
      scaleRange: DEFAULT_VIEWPORT_SCALE_RANGE,
      ...
      >
        {Renderer && (
          <Renderer
            ...
            stats={stats}
            viewport={viewport}
            transformPreviewActive={transformPreviewActive}
