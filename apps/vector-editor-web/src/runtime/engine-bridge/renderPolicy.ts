...
// Force high-zoom frames to use at least device DPR so zoomed content stays readable.
function resolveHighZoomDprOverride(input: {
  viewportScale?: number
  deviceDpr?: number
}): number | null {
  const HIGH_ZOOM_SHARPNESS_SCALE_THRESHOLD = 2

  if ((input.viewportScale ?? 1) < HIGH_ZOOM_SHARPNESS_SCALE_THRESHOLD) {
    return null
  }

  if (typeof input.deviceDpr !== 'number' || Number.isNaN(input.deviceDpr) || input.deviceDpr <= 0) {
    return null
  }

  return Math.max(1, input.deviceDpr)
}

// Resolve policy DPR with high-zoom sharpness protection layered above base LOD policy.
function resolvePolicyDpr(input: {
  ...
  const highZoomDpr = resolveHighZoomDprOverride({
    viewportScale: input.viewportScale,
    deviceDpr: input.deviceDpr,
  })
  if (highZoomDpr !== null) {
    return input.capabilityDpr === 'auto'
      ? highZoomDpr
      : Math.max(input.capabilityDpr, highZoomDpr)
  }
  ...
  const baseDpr = input.capabilityDpr === 'auto'
    ...
    : input.capabilityDpr
  // Keep DPR stable across phases so pan/zoom do not introduce policy-driven
  // fidelity drops while the snapshot path is active.
  return baseDpr
}
...
export function resolveRuntimeRenderPolicy(
  ...
  if (input.phase === 'pan') {
    const capability = resolveRuntimeLodInteractionCapability({
      ...
      phase: 'pan',
      fallback: {
        // Keep pan full-fidelity and let interaction scheduling + snapshot reuse
        // provide responsiveness instead of policy-level LOD degradation.
        quality: 'full',
    ...
    return {
      ...
      // Keep overlay fidelity aligned with base-scene fidelity during pan.
      overlayMode: 'full',
    }
  }
  ...
  if (input.phase === 'zoom') {
    const capability = resolveRuntimeLodInteractionCapability({
      ...
      phase: 'zoom',
      fallback: {
        // Keep zoom full-fidelity and rely on snapshot-first interaction
        // behavior instead of dynamic LOD policy switches.
        quality: 'full',
        interactionActive: true,
      },
    })
    return {
      ...
      // Keep overlay fidelity aligned with base-scene fidelity during zoom.
      overlayMode: 'full',
    }
  }
