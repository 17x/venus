...
export function deriveEditorUIState(options: {
  canvasRuntime: CanvasRuntimeBridgeState<import('../runtime/model/index.ts').EditorDocument>
...
function buildLayerItems(nodes: DocumentNode[]): LayerItem[] {
  ...
  const visit = (node: DocumentNode, depth: number) => {
    ...
    // AI-TEMP: diagnostic logging for layer tree debugging; remove after verifying tree structure.
    if (node.type === 'group' && node.id.startsWith('group-')) {
      console.debug(`[LayerTree] visit group depth=${depth} id=${node.id} childIds=[${nextChildIds.join(',')}] parentId=${node.parentId}`)
    }
...
function buildLayerHierarchyIndex(nodes: DocumentNode[]) {
  ...
  // AI-TEMP: diagnostic logging; remove after verifying tree structure.
  console.debug(`[LayerTree] total=${nodes.length} roots=[${roots.map((r) => `${r.id}(${r.type})`).join(',')}]`)

  return {nodeById, childIdsByParent, roots}
}
