...
const TemplatePresetPicker: FC<TemplatePresetPickerProps> = ({bg, onClose, onGenerate}) => {
  ...
  >
    <div className={cn('h-[82vh] w-[95vw] max-w-[1160px] rounded-xl border border-[var(--vector-shell-border)] bg-[var(--vector-shell-surface)] shadow-2xl', EDITOR_TEXT_BODY_CLASS)}>
      <div className={'flex h-full min-h-0 flex-col overflow-hidden'}>
        <header className={'flex items-center justify-between gap-4 border-b border-[var(--vector-shell-border)] px-4 py-3'}>
          <div>
            <h2 className={'text-sm font-semibold text-[var(--vector-shell-text)]'}>{t('ui.template.pickerTitle')}</h2>
            <p className={'mt-0.5 text-xs text-[var(--vector-shell-text-muted)]'}>{t('ui.template.pickerSubtitle')}</p>
          </div>
        ...
        <div className={'grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[1.25fr_0.75fr]'}>
          <section className={'min-h-0 border-r border-[var(--vector-shell-border)] p-3'} data-testid={TEST_IDS.templatePicker.options}>
            <h3 className={'mb-2 text-xs font-semibold text-[var(--vector-shell-text)]'}>{t('ui.template.pickerTitle')}</h3>
              ...
              <div className={'space-y-4'} role={'listbox'} aria-label={t('ui.template.pickerTitle')}>
                {(['simple-demo', 'mixed-large', 'image-heavy'] as const).map((category) => {
                  ...
                  return <section key={category}>
                    <h3 className={'mb-1.5 text-[11px] font-semibold uppercase tracking-wide text-[var(--vector-shell-text-muted)]'}>
                        ...
                        return <Button
                          ...
                          className={cn(
                            ...
                            active
                              ...
                              : 'hover:border-[var(--vector-ui-border-color-strong)]',
                          )}
                        >
                          <div className={'w-full'}>
                            <div className={'flex items-center justify-between gap-2'}>
                              <div className={'text-sm font-medium text-[var(--vector-shell-text)]'}>{preset.label}</div>
                              <span className={'rounded bg-[var(--vector-shell-hover)] px-1.5 py-0.5 text-[11px] text-[var(--vector-shell-text-muted)]'}>
          ...
          <section className={'min-h-0 flex flex-col p-3'} data-testid={TEST_IDS.templatePicker.details}>
            <h3 className={'mb-2 text-xs font-semibold text-[var(--vector-shell-text)]'}>{t('ui.template.presetDetails')}</h3>
            <div className={'scrollbar-custom min-h-0 flex-1 overflow-auto'}>
              {activePreset
                ? <div className={'rounded-lg border border-[var(--vector-shell-border)] bg-[var(--vector-shell-surface-muted)] p-3'}>
                    <div className={'text-sm font-semibold text-[var(--vector-shell-text)]'}>{activePreset.label}</div>
                    <p className={'mt-1 text-xs text-[var(--vector-shell-text-muted)]'}>{activePreset.description}</p>
                    <div className={'mt-2 text-xs text-[var(--vector-shell-text-muted)]'}>
                      {t('ui.template.categoryLabel')}: {t(CATEGORY_LABELS[activePreset.category])}
                    </div>
                    <div className={'text-xs text-[var(--vector-shell-text-muted)]'}>
                ...
                : <div className={'rounded-lg border border-dashed border-[var(--vector-shell-border)] bg-[var(--vector-shell-surface-muted)] p-3 text-xs text-[var(--vector-shell-text-muted)]'}>
        ...
        <div className={'flex shrink-0 items-center justify-end gap-2 border-t border-[var(--vector-shell-border)] px-4 py-3'} data-testid={TEST_IDS.templatePicker.footer}>
