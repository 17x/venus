...
function RightSidebarComponent(props: RightSidebarProps) {
  ...
  if (props.rightPanelMinimized) {
    return (
      <Button
        ...
        className={'absolute right-0 border round-lg border-slate-200 inline-flex h-9 items-center gap-1 rounded bg-white px-2 text-slate-700 transition-colors hover:bg-slate-100 hover:text-slate-900 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800 dark:hover:text-slate-50'}
  ...
  return (
    <div className={'border round-lg border-slate-200 pointer-events-auto h-full overflow-hidden rounded-lg'} style={{width: props.panelWidth}}>

      <aside className={'flex h-full shrink-0 flex-col overflow-hidden bg-white dark:bg-slate-900'} aria-label={t('shell.variantB.rightSidebar', 'Right sidebar')} data-testid={TEST_IDS.sidebarRight.workspace}>
