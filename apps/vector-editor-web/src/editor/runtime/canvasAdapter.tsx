...
export function CanvasViewport({
  ...
  const imageCount = React.useMemo(
    // Keep accumulator types explicit so strict mode does not infer any from external document contracts.
    () => document.shapes.reduce((count: number, shape: EditorDocument['shapes'][number]) => count + (shape.type === 'image' ? 1 : 0), 0),
    [document.shapes],
  )
...
export function Canvas2DRenderer({
  ...
  React.useEffect(() => {
    ...
    // Keep local backend read for compatibility with component props while engine ignores runtime backend selection.
    void backend
    const engine = createEngine({
      ...
      render: {
        quality: 'full',
        // Use the engine-owned clear color contract expected by current runtime options.
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      // Pass full render surface metrics through the object resize contract.
      engine.resize({
