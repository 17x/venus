...
export const engineApiModuleGroups: EngineApiModuleGroup[] = [
  ...
  {id: 'interaction', title: 'Interaction', summary: 'Selection state, marquee queries, and style-free overlay geometry.'},
...
const methodModuleNameById: Record<string, EngineApiModuleName> = {
  ...
  bringToFront: 'render',
  sendToBack: 'render',
  validateClipGraph: 'render',
  resolveClipDependencies: 'render',
  group: 'render',
  ungroup: 'render',
  addChild: 'render',
  removeChild: 'render',
  ...
  querySelectionInRect: 'interaction',
  selectInRect: 'interaction',
  getSelectionOverlay: 'interaction',
  getHoverOverlay: 'interaction',
  animate: 'animate',
  enableDebug: 'debug',
  inspect: 'debug',
  measureFrame: 'debug',
  ...
  },
  {
    ...
    apis: [
      ...
      },
      {
        id: 'frame',
        title: 'Frame',
        summary: 'Draws a bounds-owned container with its own background and children.',
        readableDescription: 'Frame is the Figma-style artboard/container model in the engine document tree. Minimal creation is type, width, height, and children. Unlike group, frame owns x/y/width/height and can render fill, stroke, radius, effects, and child nodes inside the same subtree.',
        properties: ['type: frame', 'minimal: type + width + height + children', 'pathExpansion: frame background expands like a rect; children remain separate nodes', 'id?: string', 'name?: string', 'visible?: boolean', 'locked?: boolean', 'data?: Record<string, unknown> as host metadata', 'appearance?: VenusAppearance as structured render style', 'constraints?: VenusConstraints', 'exportSettings?: readonly VenusExportSetting[]', 'blendMode?: string as multiply|screen|overlay|darken|lighten|colorDodge|colorBurn|hardLight|softLight|difference|exclusion', 'transform?: VenusTransform2D', 'rotation?: number as top-level rotation field', 'x?: number as frame bounds x', 'y?: number as frame bounds y', 'width: number as frame bounds width', 'height: number as frame bounds height', 'fill?: string as compatibility background fill shortcut', 'fills?: VenusPaint[] as ordered background fill paint list', 'stroke?: string as compatibility border stroke shortcut', 'strokes?: VenusPaint[] as ordered border stroke paint list', 'strokeWidth?: number, 0 means no stroke', 'strokeAlign?: center|inside|outside', 'strokeDashArray?: number[] as alternating dash-gap lengths', 'strokeCap?: butt|round|square', 'strokeJoin?: miter|round|bevel', 'opacity?: number', 'shadow?: EngineShadow', 'cornerRadius?: number', 'cornerRadii?: topLeft, topRight, bottomRight, bottomLeft', 'children: VenusNode[] as nested scene tree objects inside the frame subtree'],
        demo: `venus.add({type: 'frame', x: 64, y: 48, width: 280, height: 180, children: [{type: 'rect', x: 96, y: 84, width: 80, height: 56}]})`,
        demoCaption: 'The preview renders a frame background with editable bounds, appearance, and children.',
      },
      {
        id: 'group',
        title: 'Group',
      ...
      },
      {
        id: 'star',
        title: 'Star',
        summary: 'Draws closed star geometry backed by authored vertices.',
        readableDescription: 'Star is a point-authored document shape. Today it renders through the same polygon geometry path as polygon while preserving its public document type for app logic, hit metadata, and future native star parameters. Minimal creation is type, width, height, and points.',
        properties: ['type: star', 'minimal: type + width + height + points', 'pathExpansion: closed point-list path; currently normalized to engine polygon geometry', 'id?: string', 'name?: string', 'visible?: boolean', 'locked?: boolean', 'data?: Record<string, unknown> as host metadata', 'appearance?: VenusAppearance as structured render style', 'constraints?: VenusConstraints', 'exportSettings?: readonly VenusExportSetting[]', 'blendMode?: string as multiply|screen|overlay|darken|lighten|colorDodge|colorBurn|hardLight|softLight|difference|exclusion', 'transform?: VenusTransform2D', 'rotation?: number as top-level rotation field', 'x?: number as editor bounds x', 'y?: number as editor bounds y', 'width: number as editor bounds width', 'height: number as editor bounds height', 'points?: {x: number, y: number}[] as ordered outer/inner vertices and render source', 'closed?: boolean (defaults to true)', 'fill?: string as compatibility fill shortcut', 'fills?: VenusPaint[] as ordered fill paint list', 'stroke?: string as compatibility stroke shortcut', 'strokes?: VenusPaint[] as ordered stroke paint list', 'strokeWidth?: number, 0 means no stroke', 'strokeAlign?: center|inside|outside', 'strokeDashArray?: number[] as alternating dash-gap lengths', 'strokeCap?: butt|round|square', 'strokeJoin?: miter|round|bevel', 'opacity?: number', 'shadow?: EngineShadow'],
        demo: `venus.add({type: 'star', x: 86, y: 50, width: 220, height: 200, points: [{x: 196, y: 50}, {x: 222, y: 124}, {x: 306, y: 124}, {x: 238, y: 168}, {x: 264, y: 250}, {x: 196, y: 200}, {x: 128, y: 250}, {x: 154, y: 168}, {x: 86, y: 124}, {x: 170, y: 124}], fill: '#fde047', stroke: '#ca8a04', strokeWidth: 3})`,
        demoCaption: 'The preview renders an editable star as closed point geometry.',
      },
      {
        id: 'path',
        title: 'Path',
      ...
      },
      {
        ...
        properties: ['type: image', 'minimal: type + width + height + assetId', 'pathExpansion: not path-expandable; renders as image quad', 'id?: string', 'name?: string', 'visible?: boolean', 'locked?: boolean', 'data?: Record<string, unknown> as host metadata', 'appearance?: VenusAppearance as structured opacity/blend/effects style', 'constraints?: VenusConstraints', 'exportSettings?: readonly VenusExportSetting[]', 'blendMode?: string', 'transform?: VenusTransform2D', 'rotation?: number as top-level rotation field', 'x?: number as image quad top-left x', 'y?: number as image quad top-left y', 'width: number as image quad width', 'height: number as image quad height', 'assetId: string', 'assetUrl?: string as import/export URL mapping; render still uses resource.loader.resolveImage(assetId)', 'sourceRect?: {x, y, width, height}', 'naturalSize?: {width, height}', 'imageSmoothing?: boolean', 'opacity?: number'],
...
console.log(venus.inspect().backendFallback)`,
  ...
  },
  {
    ...
    apis: [
      ...
      {
        id: 'inspectImageResources',
        title: 'inspectImageResources',
        summary: 'Reports image resource resolution diagnostics.',
        readableDescription: 'Traverses image nodes in the current engine snapshot and asks the configured host resource loader whether each assetId resolves to a renderable image. The engine does not load assetUrl by itself; assetUrl is preserved for import/export mapping.',
        demo: `const resources = venus.inspectImageResources()\nconsole.log(resources.missingCount)`,
        demoCaption: 'Returns per-image resolved, missing-loader, missing-resource, or loader-error diagnostics.',
      },
      {
        id: 'fitBounds',
        title: 'fitBounds',
      ...
      },
      {
        ...
        readableDescription: 'Tests a screen-space point against the scene, respecting node order, transforms, visibility, locked state, and clip geometry by default. Returns the topmost eligible hit with node identity, bounds, center, anchor, region, and target metadata.',
        parameters: [
          ...
          {name: 'options.respectClip', type: 'boolean', defaultValue: 'true', description: 'When true, clipped-out children are removed from hit results; set false only for debug or parity checks.'},
        ],
      ...
      },
      {
        ...
        readableDescription: 'Uses the same hit engine as hitTest, but returns all eligible hits in topmost-first order while applying the same clip filtering policy. Each result includes target kind values such as shape.stroke, shape.anchor, shape.center, shape.fill, or shape.bounds.',
        parameters: [
          {name: 'point', type: '{x, y}', description: 'Point to test in screen space.'},
          {name: 'options.phase', type: 'hover | click', defaultValue: 'click', description: 'Sets the default tolerance for pointer hover or committed click selection.'},
      ...
      },
      {
        ...
        readableDescription: 'Registers a handler for events like mounted, document:changed, selection:changed, backend:fallback, render:after, hit, and destroyed. Returns an unsubscribe function.',
        parameters: [
      ...
      },
      {
        id: 'remove',
        title: 'remove',
        summary: 'Removes a document node or subtree by id.',
        readableDescription: 'Deletes a root or nested node from its current parent-local child list. The removed node\'s subtree is cleaned from the document indexes. Successful removal emits document:node-removed, document:structure-changed, and document:changed; missing ids are no-ops.',
        parameters: [
      ...
      },
      {
        id: 'getLayerOrder',
        title: 'getLayerOrder',
        summary: 'Returns ordered sibling ids for one parent.',
        readableDescription: 'Reads the current sibling order for root nodes or for one group, clip, mask, or frame parent. Larger indexes render above smaller indexes.',
        parameters: [
          {name: 'parentId', type: 'string | null', defaultValue: 'null', description: 'Parent id, or null/omitted for root nodes.'},
        ],
        demo: `const rootOrder = venus.getLayerOrder()\nconst groupOrder = venus.getLayerOrder('group-1')`,
        demoCaption: 'Reads root or parent-local layer order.',
      },
      {
        id: 'moveLayer',
        title: 'moveLayer',
      ...
      },
      {
        id: 'moveBefore',
        title: 'moveBefore',
        summary: 'Moves one node immediately before a sibling target.',
        readableDescription: 'Reorders one node within its current parent so it renders immediately before the target sibling. Cross-parent moves are rejected by returning -1.',
        parameters: [
          {name: 'id', type: 'string', description: 'Node id to move.'},
          {name: 'targetId', type: 'string', description: 'Sibling node id to move before.'},
        ],
        demo: `venus.moveBefore('card', 'background')`,
        demoCaption: 'Places card directly before another sibling.',
      },
      {
        id: 'moveAfter',
        title: 'moveAfter',
        summary: 'Moves one node immediately after a sibling target.',
        readableDescription: 'Reorders one node within its current parent so it renders immediately after the target sibling. Cross-parent moves are rejected by returning -1.',
        parameters: [
          {name: 'id', type: 'string', description: 'Node id to move.'},
          {name: 'targetId', type: 'string', description: 'Sibling node id to move after.'},
        ],
        demo: `venus.moveAfter('card', 'background')`,
        demoCaption: 'Places card directly after another sibling.',
      },
      {
        id: 'bringForward',
        title: 'bringForward',
      ...
      {
        id: 'validateClipGraph',
        title: 'validateClipGraph',
        summary: 'Validates clip references and clip-path geometry.',
        readableDescription: 'Checks clip metadata for duplicate node ids, missing references, self references, cycles, invalid rules, and invalid inline clip geometry. Defaults to the current document snapshot.',
        parameters: [
          {name: 'snapshot', type: 'EngineSceneSnapshot', defaultValue: 'current snapshot', description: 'Optional scene snapshot to validate instead of the current document.'},
        ],
        demo: `const result = venus.validateClipGraph()`,
        demoCaption: 'Returns clip issues and dependency edges.',
      },
      {
        id: 'resolveClipDependencies',
        title: 'resolveClipDependencies',
        summary: 'Returns direct and transitive clip node ids for one node.',
        readableDescription: 'Follows clipNodeId links from one render node through its clip chain. The order matches traversal from the node outward through referenced clips.',
        parameters: [
          {name: 'nodeId', type: 'string', description: 'Node id used as the clip chain starting point.'},
          {name: 'snapshot', type: 'EngineSceneSnapshot', defaultValue: 'current snapshot', description: 'Optional scene snapshot to inspect instead of the current document.'},
        ],
        demo: `const clipIds = venus.resolveClipDependencies('image-1')`,
        demoCaption: 'Reads the clip dependency chain for one node.',
      },
      {
        id: 'undo',
        title: 'undo',
      ...
      },
      {
        ...
        readableDescription: 'Creates a structure-only group from direct siblings under the same parent. Before grouping, the engine removes selected descendants when their ancestor is also selected and sorts ids by document order. Child geometry is preserved in the parent coordinate space, visual bounds stay stable, and cross-parent grouping is rejected until explicit tree transactions are used.',
        parameters: [
      ...
      },
      {
        ...
        readableDescription: 'Replaces a structure-only group with its children in the same parent. Children already live in parent coordinate space, so ungrouping is a pure tree operation. The removed group wrapper\'s own name, data, appearance, opacity, blend, effects, constraints, and export settings are discarded rather than baked into children.',
        parameters: [
      ...
      },
      {
        id: 'addChild',
        title: 'addChild',
        summary: 'Adds a child node to a frame, group, clip, or mask container.',
        ...
        parameters: [
          {name: 'parentId', type: 'string', description: 'Parent frame/group/clip/mask id.'},
      ...
      },
      {
        id: 'removeChild',
        title: 'removeChild',
        summary: 'Removes a child node from a frame, group, clip, or mask container.',
      ...
      },
      {
        id: 'reparent',
        title: 'reparent',
        summary: 'Moves one node into another parent-local child list.',
        readableDescription: 'Moves a node into a frame, group, clip, mask, or the root layer. The operation is a structure transaction: it rejects moving a node into itself or its descendant, preserves authored geometry in the current Venus document model, emits document:structure-changed with reason reparent, and records history when the history module is installed.',
        parameters: [
          {name: 'id', type: 'string', description: 'Node id to move.'},
          {name: 'parentId', type: 'string | null', description: 'Target parent id, or null for the root layer.'},
          {name: 'index', type: 'number', description: 'Optional insertion index inside the target parent-local child list.'},
        ],
        demo: `const result = venus.reparent('rect-1', 'group-1', 0)`,
        demoCaption: 'Returns reparent mutation metadata with old parent, new parent, indexes, and target order.',
      },
      {
        id: 'getSelection',
        title: 'getSelection',
        summary: 'Returns the current selected node ids.',
        readableDescription: 'Returns a stable snapshot of the interaction-module selection set. Mutating the returned Set does not mutate engine state.',
        demo: `const selectedIds = [...venus.getSelection()]`,
        demoCaption: 'Reads the current selection without changing it.',
      },
      {
        id: 'setSelection',
        title: 'setSelection',
        summary: 'Replaces the current selection.',
        readableDescription: 'Replaces the interaction-module selection set with the supplied document node ids. Duplicate ids collapse through Set semantics and no-op replacements do not notify listeners.',
        parameters: [
          {name: 'ids', type: 'readonly string[]', description: 'Document node ids to select.'},
        ],
        demo: `venus.setSelection(['rect-1', 'ellipse-2'])`,
        demoCaption: 'Sets the exact selection set.',
      },
      {
        id: 'select',
        title: 'select',
      ...
      {
        id: 'querySelectionInRect',
        title: 'querySelectionInRect',
        summary: 'Returns selectable ids inside or intersecting a rectangle.',
        readableDescription: 'Queries document nodes by world/document rectangle by default, or by screen rectangle when coordinateSpace is screen. It supports contain/intersect matching and locked, hidden, or container filtering.',
        parameters: [
          {name: 'rect', type: '{x, y, width, height}', description: 'Query rectangle. Negative width/height are normalized.'},
          {name: 'options.coordinateSpace', type: 'document | screen', defaultValue: 'document', description: 'Coordinate space of the rectangle.'},
          {name: 'options.mode', type: 'intersect | contain', defaultValue: 'intersect', description: 'Whether partial overlap is enough or full containment is required.'},
          {name: 'options.includeLocked', type: 'boolean', defaultValue: 'false', description: 'Whether locked nodes may match.'},
          {name: 'options.includeHidden', type: 'boolean', defaultValue: 'false', description: 'Whether hidden nodes may match.'},
          {name: 'options.includeContainers', type: 'boolean', defaultValue: 'true', description: 'Whether group/clip/mask container bounds may match.'},
        ],
        demo: `const ids = venus.querySelectionInRect({x: 0, y: 0, width: 240, height: 180}, {mode: 'contain'})`,
        demoCaption: 'Reads marquee candidates without mutating selection.',
      },
      {
        id: 'selectInRect',
        title: 'selectInRect',
        summary: 'Applies a rectangle query to the current selection.',
        readableDescription: 'Runs querySelectionInRect and applies the result through replace, add, subtract, or toggle semantics. Returns the final selection ids after the operation.',
        parameters: [
          {name: 'rect', type: '{x, y, width, height}', description: 'Query rectangle. Negative width/height are normalized.'},
          {name: 'options.selectionMode', type: 'replace | add | subtract | toggle', defaultValue: 'replace', description: 'How query results modify the current selection.'},
        ],
        demo: `const selected = venus.selectInRect({x: 0, y: 0, width: 240, height: 180}, {selectionMode: 'add'})`,
        demoCaption: 'Applies marquee selection through the interaction module.',
      },
      {
        id: 'getSelectionOverlay',
        title: 'getSelectionOverlay',
        summary: 'Returns style-free selection overlay geometry.',
        readableDescription: 'Resolves aggregate selection bounds and outline geometry through the interaction module. The engine owns geometry and revision; host apps such as Vector own color, visibility, handles, and product styling.',
        parameters: [
          {name: 'options.ids', type: 'readonly string[]', defaultValue: 'current selection', description: 'Optional ids to inspect instead of the current selection.'},
        ],
        demo: `const overlay = venus.getSelectionOverlay()
if (overlay) {
  console.log(overlay.bounds, overlay.outline.kind)
}`,
        demoCaption: 'Reads selection geometry without choosing visual styling.',
      },
      {
        id: 'getHoverOverlay',
        title: 'getHoverOverlay',
        summary: 'Returns style-free hover overlay geometry.',
        readableDescription: 'Resolves hover outline geometry from a known node id or from a topmost hit-test point. Passing point uses venus.hitTest and therefore also requires the hitTest module; passing nodeId only requires interaction.',
        parameters: [
          {name: 'options.nodeId', type: 'string', description: 'Known document node id to outline.'},
          {name: 'options.point', type: '{x, y}', description: 'Screen-space point used to resolve topmost hit when nodeId is absent.'},
          {name: 'options.hitOptions', type: 'VenusHitTestOptions', defaultValue: '{}', description: 'Hit-test options used with point.'},
        ],
        demo: `const hover = venus.getHoverOverlay({nodeId: 'rect-1'})`,
        demoCaption: 'Reads hover geometry while leaving styling to the host app.',
      },
      {
        id: 'applyDropShadow',
      ...
      },
      {
        ...
        readableDescription: 'Delegates to the export module and serializes the mounted canvas to a PNG data URL. This is the preferred explicit export API; toPNG remains a compatibility alias.',
        parameters: [
          {name: 'options', type: 'VenusPngExportOptions', description: 'Optional PNG export settings.'},
          {name: 'options.scale', type: 'number', defaultValue: '1', description: 'Output scale factor.'},
          {name: 'options.background', type: 'string', defaultValue: 'transparent', description: 'Background color composited before export.'},
        ],
        demo: `const url = await venus.exportPNG({ scale: 2 })`,
      ...
      },
      {
        ...
        readableDescription: 'Delegates to the export module and serializes the mounted canvas to a JPEG data URL with quality and background options. This is the preferred explicit export API; toJPEG remains a compatibility alias.',
        parameters: [
          {name: 'options', type: 'VenusJpegExportOptions', description: 'Optional JPEG export settings.'},
          {name: 'options.scale', type: 'number', defaultValue: '1', description: 'Output scale factor.'},
        ...
        demo: `const url = await venus.exportJPEG({ quality: 0.9, background: '#ffffff' })`,
      ...
      },
      {
        ...
        parameters: [
          {name: 'options', type: 'VenusSvgExportOptions', description: 'Optional SVG export settings.'},
          ...
          {name: 'options.pretty', type: 'boolean', description: 'Whether to emit line breaks between top-level SVG nodes.'},
        ],
        demo: `const svg = await venus.exportSVG({ embedImages: true })`,
        demoCaption: 'Exports document to SVG.',
      },
      {
        id: 'exportNode',
        title: 'exportNode',
        summary: 'Exports one document node and its subtree as SVG.',
        readableDescription: 'Creates a scoped snapshot for one node id, trims the SVG viewBox to content by default, and serializes the result through the export module.',
        parameters: [
          {name: 'id', type: 'string', description: 'Document node id to export.'},
          {name: 'options', type: 'VenusScopedSvgExportOptions', defaultValue: '{}', description: 'SVG serialization and cropping options.'},
        ],
        demo: `const svg = await venus.exportNode('card')`,
        demoCaption: 'Exports one node subtree.',
      },
      {
        id: 'exportSelection',
        title: 'exportSelection',
        summary: 'Exports the current selected nodes as SVG.',
        readableDescription: 'Creates a scoped snapshot from the interaction selection, trims the SVG viewBox to content by default, and serializes the result through the export module.',
        parameters: [
          {name: 'options', type: 'VenusScopedSvgExportOptions', defaultValue: '{}', description: 'SVG serialization and cropping options.'},
        ],
        demo: `const svg = await venus.exportSelection()`,
        demoCaption: 'Exports the current selection as SVG.',
      },
      {
        id: 'toPNG',
        title: 'toPNG',
    ...
    apis: [
      {
        ...
        properties: [
          ...
          'render:before: {revision}',
          'render:after: {revision}',
          'destroyed: {}',
          'backend:fallback: {from, to, reason}',
          'hit: {point, phase, tolerance, result}',
          'selection:changed: {selection, previousSelection, added, removed}',
          'document:changed: {revision, node?, affectedNodeIds?, reason?}',
          'document:node-added: {revision, nodeIds, parentId}',
          'document:node-updated: {revision, nodeId, changedProperties, invalidation}',
          'document:node-removed: {revision, nodeIds, parentId}',
          'document:structure-changed: {revision, parentId, affectedParentIds?, affectedNodeIds, order, reason}',
          'layer:changed: VenusLayerMutationResult',
        ],
        methods: [
          {name: 'venus.on', description: 'Register a handler for one event channel and return an unsubscribe function.', parameters: [{name: 'eventName', type: 'VenusEventName', description: 'Event channel such as mounted, document:changed, selection:changed, backend:fallback, render:after, hit, or destroyed.'}, {name: 'handler', type: '(event) => void', description: 'Callback invoked when the event fires.'}]},
