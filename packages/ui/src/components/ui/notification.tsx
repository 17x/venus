...
export function NotificationProvider({children}: {children: ReactNode}) {
  ...
  return (
    <NotificationContext.Provider value={value}>
      {children}
      <div className="pointer-events-none fixed right-3 top-3 z-[2000] flex flex-col gap-2">
        {notifications.map((item) => (
          <div
            key={item.id}
            className="pointer-events-auto rounded border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 shadow"
            role="status"
          >
            {item.comp}
          </div>
