...
test('renderScheduler supports injected now/schedule/cancel adapters', async () => {
  let now = 0
  let nextHandle = 1
  ...
  const scheduler = createEngineRenderScheduler({
    ...
    now: () => now,
    // Use an injected scheduler so engine loop callers can avoid browser globals.
  ...
  // Keep queue start non-zero so diagnostics report measured wait time.
  now = 1
  scheduler.request('normal')
  assert.equal(scheduled.size, 1)

  const [handle, callback] = scheduled.entries().next().value ?? []
  assert.ok(handle, 'expected one injected frame callback')
  scheduled.delete(handle)
  now = 6
  callback()
  await Promise.resolve()
  ...
  assert.equal(scheduler.getDiagnostics().lastQueueWaitMs, 5)
})