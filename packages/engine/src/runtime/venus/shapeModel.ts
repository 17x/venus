...
export const VENUS_SHAPE_MODEL_SPECS: readonly VenusShapeModelSpec[] = [
  ...
  },
  {
    ...
    proxy: 'VenusFrameProxy',
    inherits: ['VenusNodeProxy'],
    minimalCreate: ['type', 'width', 'height', 'children'],
    bounds: 'x/y/width/height are authored frame bounds. Unlike group, frame bounds are not derived from children.',
    geometry: ['x', 'y', 'width', 'height', 'children'],
    pathExpansion: 'Frame renders as a container with an internal rectangle background plus child nodes.',
    commonRender: ['fill', 'stroke', 'multi fill paints', 'multi stroke paints', 'subtree opacity', 'blend/effects on composed children'],
  },
  {
    type: 'group',
    family: 'container',
  ...
  },
  {
    ...
    inherits: ['VenusNodeProxy'],
    minimalCreate: ['type', 'width', 'height', 'points'],
    bounds: 'x/y/width/height are the editor bounds for transforms and proxy resizing. points hold the star vertex geometry and are rescaled when bounds are patched.',
    geometry: ['points', 'closed'],
    pathExpansion: 'Currently normalized to the engine polygon render shape. A future native star model may store inner radius, outer radius, and point count.',
    commonRender: ['fill', 'stroke', 'multi fill paints', 'multi stroke paints', 'stroke alignment', 'dash/cap/join', 'opacity', 'blend/effects'],
  },
  {
    type: 'path',
