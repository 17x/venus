...
export const VENUS_SHAPE_MODEL_SPECS: readonly VenusShapeModelSpec[] = [
  ...
  },
  {
    ...
    inherits: ['VenusNodeProxy'],
    minimalCreate: ['type', 'width', 'height'],
    bounds: 'x/y are the start point; width/height are the end-point delta. Setting bounds rewrites endpoint geometry when explicit points exist.',
    geometry: ['x/y start point', 'width/height end delta'],
    pathExpansion: 'Already equivalent to an open two-point path.',
