...
/**
 * Handles withFakePerformanceNow.
...
test('tileManager helpers resolve stable keys, zoom buckets, and world bounds', () => {
  ...
  assert.equal(getZoomBucket(40, [0.01, 0.1, 1, 10, 100]), 100)
  assert.deepEqual(getActiveZoomBuckets(10, [0.01, 0.1, 1, 10, 100], 2), [0.1, 1, 10, 100])
...
test('EngineTileCache markAllTilesDirty invalidates every cached tile entry', () => {
  withFakePerformanceNow((advance) => {
    const cache = new EngineTileCache({
      enabled: true,
      tileSizePx: 512,
    ...
    advance(1)
    cache.upsertEntry({zoomLevel: 3, gridX: 0, gridY: 0, textureId: 1, textureBytes: 256})
    advance(2)
    cache.upsertEntry({zoomLevel: 4, gridX: 1, gridY: 0, textureId: 2, textureBytes: 256})

    cache.markAllTilesDirty()

    const dirtyTiles = cache.getDirtyTiles()
    assert.equal(dirtyTiles.length, 2)
    assert.deepEqual(dirtyTiles, [
      {zoomLevel: 3, gridX: 0, gridY: 0},
      {zoomLevel: 4, gridX: 1, gridY: 0},
    ])
