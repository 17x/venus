 ...
 * so auto-selected WebGL does not regress to blank output before native pipeline lands.
...
export function createWebGLBackendAdapter(
  ...
  // AI-TEMP: WebGL adapter is currently stubbed; remove when native WebGL present path is implemented; ref DEX-065.
  ...
  return createNoopBackendAdapter("webgl", hooks);
}
