...
`engine.runtime.model` 是位于 `engine.runtime.resource` 之上的通用 runtime 层。Resource API 负责 residency 与 decode state；model API 负责可复用 scene assets、runtime instances、LOD 选择与 diagnostics，不携带场景专有名称。
...
- Model assets 使用与 glTF ingestion 共享的 canonical `EngineSceneAsset` 容器。
- Model instances 会规范化 translation、rotation、scale、可选 color 与可选 LOD override。
- `getInstances({ cameraPosition })` 返回按 instance id 排序的 deterministic snapshots，并带 resolved LOD levels。
- Diagnostics 会报告 registered model count、instance count、instanced model count、missing model references、LOD-resolved instances 与 mesh node count。
- Playground S10 会注册通用 vehicle、pedestrian、lamp、sun、moon model assets，并通过 `engine.runtime.model` 驱动 route diagnostics。
