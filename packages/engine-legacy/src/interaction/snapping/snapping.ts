...
/**
 * Handles resolveEngineMoveSnapPreview.
 * @param preview preview parameter.
...
export function resolveEngineMoveSnapPreview<TShape extends EngineMoveSnapShape>(
  ...
  const xSources = [
    ...
    {value: (previewBounds.minX + previewBounds.maxX) / BOUNDS_CENTER_DIVISOR, kind: 'center' as const},
    {value: previewBounds.maxX, kind: 'edge-max' as const},
  ]
  const ySources = [
    {value: previewBounds.minY, kind: 'edge-min' as const},
    {value: (previewBounds.minY + previewBounds.maxY) / BOUNDS_CENTER_DIVISOR, kind: 'center' as const},
...
/**
 * Handles resolveEngineSnapGuideLines.
 * @param options Options object for this operation.
 */
...
/**
 * Handles resolvePreviewBounds.
 * @param shapes shapes parameter.
 */
...
/**
 * Handles collectBoundsSnapCandidates.
 * @param scene Scene snapshot.
 * @param excludeIds excludeIds parameter.
 * @param previewBounds previewBounds parameter.
