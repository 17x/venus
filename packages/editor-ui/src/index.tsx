 ...
 *
 * Represents the full state and interaction contract between the editor UI
 * shell and the rendering/runtime layer.
 *
 * - document: Core document model (canvas size, metadata, etc.)
 * - shapes: Snapshot of all scene shapes (likely from shared memory)
 * - stats: Runtime statistics (selection, hover, versioning, etc.)
 * - runtimeReady: Whether the worker/rendering runtime is initialized
 * - sabSupported: Whether SharedArrayBuffer is supported in the environment
 * - onPointerMove: Pointer move event handler (delegated to runtime)
...
/**
 * Creates a simple rectangle payload so the UI can exercise the insert command
 * path end-to-end without depending on a dedicated shape panel yet.
 */
function createRectangleInsertion(document: EditorDocument, shapeCount: number) {
  const ordinal = shapeCount + 1
  const offset = (shapeCount % 6) * 24

  return {
    type: 'shape.insert' as const,
    shape: {
      id: `shape-${ordinal}-${Date.now()}`,
      type: 'rectangle' as const,
      name: `rectangle ${ordinal}`,
      x: 120 + offset,
      y: 120 + offset,
      width: Math.max(96, Math.round(document.width * 0.2)),
      height: Math.max(96, Math.round(document.height * 0.12)),
    },
  }
}

function handleInsertRectangle(
  document: EditorDocument,
  shapeCount: number,
  onCommand: (command: EditorRuntimeCommand) => void,
) {
  const command = createRectangleInsertion(document, shapeCount)
  console.debug('EDITOR-UI insert rectangle', command)
  onCommand(command)
}
            ...
            <header className="topbar">
              ...
              <div className="topbar-actions">
                ...
                <button
                  ...
                  onClick={() => handleInsertRectangle(document, stats.shapeCount, onCommand)}
                >
                  Insert Rectangle
                </button>
                <button
...
export { EditorMenuBar } from './menu/index.ts'
export { EditorMenuItemView } from './menu/index.ts'
export type {
