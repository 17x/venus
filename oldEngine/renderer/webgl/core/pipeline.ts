/**
 * Renderer/WebGL pipeline module.
 * Owns quad shader pipeline creation and low-level draw submission primitives.
 * Does not decide scene policy, LOD strategy, or tile scheduling.
 */
import type { EngineRenderFrame } from '../../types/index.ts'

const WEBGL_POSITION_COMPONENT_COUNT = 2
const RGBA_BLUE_CHANNEL_INDEX = 2
const RGBA_ALPHA_CHANNEL_INDEX = 3
const WEBGL_QUAD_VERTEX_COUNT = 4
...
/**
 * Handles createViewportMatrixForRender.
 * @param scale Scale value.
 * @param offsetX offsetX parameter.
 * @param offsetY offsetY parameter.
 */
...
/**
 * Handles createWebGLQuadPipeline.
 ...
 * @param texture texture parameter.
 * @param flipTextureY flipTextureY parameter.
 */
export function drawWebGLPacket(
  ...
  context.vertexAttribPointer(pipeline.attributePosition, WEBGL_POSITION_COMPONENT_COUNT, context.FLOAT, false, 0, 0)
  ...
  context.uniform4f(
    ...
    color[0],
    color[1],
    color[RGBA_BLUE_CHANNEL_INDEX],
    color[RGBA_ALPHA_CHANNEL_INDEX] * opacity,
  )
  ...
  context.drawArrays(context.TRIANGLE_STRIP, 0, WEBGL_QUAD_VERTEX_COUNT)
  return 1
}

/**
 * Handles disposeWebGLQuadPipeline.
 * @param context Rendering context.
 * @param pipeline pipeline parameter.
 */
...
/**
 * Handles createProgram.
 * @param context Rendering context.
 * @param vertexShader vertexShader parameter.
 * @param fragmentShader fragmentShader parameter.
 */
