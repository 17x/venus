/**
 * Renderer/WebGL texture module.
 * Owns texture upload/reuse/downsample rules and texture cache contracts.
 * Does not orchestrate frame-level render order or capability policy.
 */
...
/**
 * Handles resolveDisplayPixelRatio.
...
const MAX_IMAGE_TEXTURE_UPLOAD_BUDGET_MIB = 16
const PENDING_IMAGE_TEXTURE_ESTIMATE_MIB = 4
const IMAGE_RASTER_REUSE_HEADROOM = 1.25
const IMAGE_DOWNSAMPLE_RASTER_THRESHOLD = 0.99
const IMAGE_DOWNSAMPLE_DIMENSION_THRESHOLD = 0.85

export const MAX_IMAGE_TEXTURE_UPLOAD_BYTES_PER_FRAME = MAX_IMAGE_TEXTURE_UPLOAD_BUDGET_MIB * BYTES_PER_KIBIBYTE * BYTES_PER_KIBIBYTE
...
/**
 * Handles resolveTextCacheKey.
 * @param packet packet parameter.
 */
...
/**
 * Handles resolveTextRasterScale.
 * @param frame Current render frame.
 */
...
/**
 * Handles countPendingImageTextureEstimate.
...
export function countPendingImageTextureEstimate(
  ...
  for (const packet of packets) {
    ...
    estimatedBytes += PENDING_IMAGE_TEXTURE_ESTIMATE_MIB * BYTES_PER_KIBIBYTE * BYTES_PER_KIBIBYTE
  }
...
/**
 * Handles canReuseTextTexture.
 * @param cached cached parameter.
 * @param textCacheKey textCacheKey parameter.
 * @param textRasterScale textRasterScale parameter.
 */
...
/**
 * Handles canReuseInteractiveTextTexture.
 * @param cached cached parameter.
 * @param textCacheKey textCacheKey parameter.
 */
...
/**
 * Handles resolveImageCacheKey.
 * @param assetId assetId parameter.
 */
...
/**
 * Handles resolveImageRasterScale.
...
export function resolveImageRasterScale(
  ...
  return Math.min(1, Math.max(widthRatio, heightRatio) * IMAGE_RASTER_REUSE_HEADROOM)
}

/**
 * Handles canReuseImageTexture.
 * @param cached cached parameter.
 * @param imageCacheKey imageCacheKey parameter.
 * @param imageRasterScale imageRasterScale parameter.
 */
...
/**
 * Handles createImageUploadSource.
 * @param source source parameter.
 * @param size size parameter.
 * @param rasterScale rasterScale parameter.
...
export function createImageUploadSource(
  ...
  const shouldDownsample =
    rasterScale < IMAGE_DOWNSAMPLE_RASTER_THRESHOLD &&
    (targetWidth <= size.width * IMAGE_DOWNSAMPLE_DIMENSION_THRESHOLD || targetHeight <= size.height * IMAGE_DOWNSAMPLE_DIMENSION_THRESHOLD)
...
/**
 * Handles createImageUploadSurface.
...
/**
 * Handles resolvePacketTextureSourceRect.
 * @param worldBounds worldBounds parameter.
 * @param frame Current render frame.
 */
...
export function resolveImageTexture(
  ...
  const uploadBytes = uploadWidth * uploadHeight * BYTES_PER_PIXEL_RGBA
  const downsampledBytesSaved = Math.max(0, width * height * BYTES_PER_PIXEL_RGBA - uploadBytes)
...
/**
 * Handles resolveCanvasImageSourceSize.
 * @param source source parameter.
 */
