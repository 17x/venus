...
function resolveChangedFiles(baseRef) {
  ...
  try {
    ...
    try {
      ...
      console.warn('[file-shape:warn] unable to resolve changed files; no files checked')
      return []
    }
  }
}
...
function resolveAllScopeFiles(roots) {
  ...
  return files.map((absPath) => path.relative(repoRoot, absPath).replace(/\\/g, '/'))
}
...
function runGuard(options) {
  ...
  for (const relPath of files) {
    ...
    if (lineCount > SOFT_LIMIT) {
      warnings.push(`file-shape soft limit exceeded (${lineCount}): ${relPath}`)
    }
  }
  ...
  if (errors.length > 0) {
    ...
    return {ok: false, filesChecked: files.length, warnings: warnings.length, errors: errors.length}
  }

  console.log(
    `[file-shape] ok (scope=${options.scope}, mode=${options.changedOnly ? 'changed' : 'all'}, files=${files.length}, warnings=${warnings.length})`,
  )
  return {ok: true, filesChecked: files.length, warnings: warnings.length, errors: 0}
}
